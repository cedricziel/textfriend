...
struct LanguageDetectionTests {
  @Test(arguments: [
    ...
    ("prod.tfvars", Language.hcl),
    ("config.hcl", Language.hcl),
    ("schema.sql", Language.sql),
    ("fix.patch", Language.diff),
    ("changes.diff", Language.diff),
    ("app.log", Language.log),
    ("jobs.cron", Language.crontab),
    ("Localizable.strings", Language.strings),
    ("data.csv", Language.csv),
    ("data.tsv", Language.csv),
